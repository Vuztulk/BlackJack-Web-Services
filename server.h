...
typedef struct game{
	...
	blackJackns__tDeck player1LastDeck, player2LastDeck;
	int turnoFinalizado;
	int player_reset;
	
	pthread_mutex_t statusMutex;
	pthread_mutex_t player1NameMutex;
	pthread_mutex_t player2NameMutex;
	pthread_mutex_t turnoMutex;
	pthread_mutex_t resetGameMutex;

	pthread_cond_t resetGameCond;
    pthread_cond_t getStatusCond;
}tGame;
...
tPlayer calculateRandomPlayer(void);